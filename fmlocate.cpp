				...
				for(i=0;i<nqrys;i++) {
					if (FMIdx->SanityCheck(queries[i],strlen((char*)queries[i]),0)) {
						...
						if (extract)
							for(j=0;j<matches;j++)
                fprintf(stdout,"%d %lu\n",result[j],
                          result[j]+strlen((char*)queries[i])-1);
						else
							for(j=0;j<matches;j++)
			...
			case 1:
				for(i=0;i<nqrys;i++) {
					if (FMIdx->SanityCheck(queries[i],strlen((char*)queries[i]),1)) {
						list <uint32_t*>* results = FMIdx->Search1Error(queries[i],
                                                  strlen((char*)queries[i]));
			...
			case 2:
				for(i=0;i<nqrys;i++) {
					if (FMIdx->SanityCheck(queries[i],strlen((char*)queries[i]),2)) {
						list <uint32_t*>* results = FMIdx->Search2Error(queries[i],
                                                    strlen((char*)queries[i]));
						if (!extract) fprintf(stdout,"%s (%d) : ",queries[i],sumResults(results));
						while (!results->empty()) {
							uint32_t* arr=results->front();
