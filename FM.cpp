...
FM::build(uint8_t* T,uint32_t n,uint32_t samplerate) {
   ...
   for(i=0;i<n;i++) {
    ...
    MapperNone * map2 = new MapperNone();
    BitSequenceBuilder * bsb2 = new BitSequenceBuilderRRR(RRR_SAMPLERATE);
    T_bwt_reverse = new WaveletTreeNoptrs((uint32_t*)X_bwt_reverse,n,
        sizeof(uint8_t)*8,bsb2,map2,true);
    //free(X_bwt_reverse);
...
/*
 *This Function gets a ragne of matches, 'sp' 'ep'
 *and return an array with their locations in the file.
 */
uint32_t*
 ...
 */
int FM::searchHelper(uint8_t myChar, uint32_t& sp, uint32_t& ep, WaveletTreeNoptrs* bwt_r, uint32_t& sp_r, uint32_t& ep_r) {
 ...
 */
uint32_t
FM::backSearchHelper(uint8_t myChar, WaveletTreeNoptrs* bwt, uint32_t& sp, uint32_t& ep) {

	 sp = C[myChar] + bwt->rank(myChar, sp-1); /* LF Mapping */
	 ep = C[myChar] + bwt->rank(myChar, ep)-1; /* LF Mapping */
 ...
 */
uint32_t
FM::backSearchHelperNoUpdate(uint8_t myChar, WaveletTreeNoptrs* bwt, uint32_t sp, uint32_t ep) {

	sp = C[myChar] + bwt->rank(myChar, sp-1); 	/* LF Mapping */
	ep = C[myChar] + bwt->rank(myChar, ep)-1;
...
/**
 *This function does a backword search on a number of chars, from 'pos'
 *to 'endPostion' and updates the ranges.
 **/
...
FM::Search1Error(uint8_t* pattern,uint32_t m) {
	...
	//Case B: Error is in  [x..m]. resetting everything.
	backup->empty();
	...
	if ((stop/samplerate+1) == ((n-1)/samplerate+1))
	   skip = n-1 - stop;
		...
		if (((samplerate-stop)%samplerate)==0)
			...
			skip = suffixes[this->sampled->rank1(j)-1]-stop-1;
...
void FM::deb() {
	...
	for (uint32_t i=0;i<this->n;i++) {
		...
		printf("%d\t%d\t",i,sa);
		uint8_t bwt = this->T_bwt->access(i);
		if (bwt==0)
			printf("-NULL-\t");
		else
...
void FM::debr() {
	...
	for (uint32_t i=0;i<this->n;i++) {
		...
		printf("%d\t%d\t",i,sa);
		uint8_t bwt = this->T_bwt_reverse->access(i);
		if (bwt==0)
			printf("-NULL-\t");
		else
			printf("%c\t",remap_reverse[bwt]);
